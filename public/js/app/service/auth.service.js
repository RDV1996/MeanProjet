...
var AuthService = /** @class */ (function () {
    ...
    AuthService.prototype.getUserById = function (id) {
        ...
        return this.http.get('https://postsite.herokuapp.com/user/' + id, { headers: headers })
            ...
            .catch(function (error) { return Observable.throw("Error in getUserById in auth.Service.ts"); });
    };
    ...
    AuthService.prototype.setUsers = function (data) {
        var users = new Array();
        for (var _i = 0, _a = data.user; _i < _a.length; _i++) {
            var useritem = _a[_i];
            var user = this.setTempUser(useritem);
            users.push(user);
        }
        return users;
    };
    ...
    AuthService.prototype.isUserAdmin = function () {
        var _this = this;
        return this.typegebruikerService.getTypeById(this.user.typeGebruiker).subscribe(function (data) {
            _this.typegebruikerService.currentType = new typeGebruiker(data._id, data.typeNaam, data.omschrijving);
            _this.typegebruikerService.TypeGebruikerChanged.emit();
        });
    };
    ...
    AuthService.prototype.getByName = function (name) {
        var headers = new Headers({ 'Content-type': 'application/json' });
